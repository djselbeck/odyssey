...
public class GaplessPlayer {
	...
	public void play(String uri, int jumpTime) throws PlaybackException {
		...
		audioEffectIntent.putExtra(AudioEffect.EXTRA_PACKAGE_NAME,
				mPlaybackService.getPackageName());
		mPlaybackService.sendBroadcast(audioEffectIntent);
		mCurrentMediaPlayer.setAuxEffectSendLevel(1.0f);
		mPrimarySource = uri;
		mCurrentMediaPlayer
				.setOnCompletionListener(new TrackCompletionListener());
	...
	public void seekTo(int position) {
		try {
			if (mCurrentMediaPlayer != null && mCurrentPrepared
					&& position < mCurrentMediaPlayer.getDuration()) {
	...
	private class TrackCompletionListener implements
	...
	private class MediaPlayerErrorListner implements
